...
// Upstream requests must never hang the RPC: abort after this long.
...
async function fetchSiliconFlowBalance(apiKey, signal) {
  ...
  if (cookie) {
    ...
    try {
      ...
      const res = await fetch(SILICONFLOW_PROFILE_PEEK_API, { headers, signal: guard.signal })
      ...
      // Cookie channel failed (expired cookie, WAF, timeout, ...): fall
      // through to the open API rather than failing the whole query.
...
export function apply(ctx) {
  ...
  async function handleSnapshot(payload, signal) {
    ...
    try {
      ...
      const providerChanged = state.lastProvider !== undefined && state.lastProvider !== provider

      // Baseline & spend ledger semantics:
      // - New day or currency switch: full reset (baseline = current balance,
      //   spend = 0).
      // - Provider switch on the same day: keep today's accumulated spend
      //   (the user really spent it), but reset the day-start baseline to the
      //   new provider's current balance and seed prevTotal with it, so the
      //   DeepSeek↔SiliconFlow balance difference is never counted as spend.
      // - Otherwise: carry both over.
      let dayStart
      let spent
      let prevTotal
      if (!sameDay || !sameCurrency) {
        dayStart = balance.total
        spent = 0
        prevTotal = balance.total
