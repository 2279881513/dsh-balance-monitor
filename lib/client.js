...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      'settings.sf.cookie': '硅基流动登录 Cookie',
      'settings.sf.cookie.placeholder': '粘贴 Cookie 或 cURL 命令…',
      'settings.sf.cookie.hint': '用于查询硅基流动真实余额(网页接口);可直接粘贴 DevTools 复制的 cURL 命令,自动提取 Cookie。不填则回退用 API Key。留空保存 = 清除。',
      'settings.sf.cookie.howto': '获取:登录 cloud.siliconflow.cn → F12 → Network → 刷新 → 右键任意请求 → Copy as cURL → 整段粘贴到这里。',
      'settings.sf.cookie.configured': '已配置',
      'settings.sf.cookie.unset': '未配置',
      'settings.sf.subjectId': '硅基流动 SubjectId',
      'settings.sf.subjectId.placeholder': '如 5cjcchom7v',
      'settings.sf.subjectId.hint': '账户 ID;控制台执行 window.subjectInfo.subjectId 可查看。留空保存 = 清除。',
    };
    const en = {
      ...
      'settings.sf.cookie': 'SiliconFlow login cookie',
      'settings.sf.cookie.placeholder': 'Paste cookie or cURL…',
      'settings.sf.cookie.hint': 'Used to query the real SiliconFlow balance (web API); you can paste a cURL command copied from DevTools and the cookie is extracted automatically. Falls back to the API key when empty. Leave empty to clear.',
      'settings.sf.cookie.howto': 'How to get it: sign in at cloud.siliconflow.cn → F12 → Network → refresh → right-click any request → Copy as cURL → paste the whole command here.',
      'settings.sf.cookie.configured': 'Configured',
      'settings.sf.cookie.unset': 'Not set',
      'settings.sf.subjectId': 'SiliconFlow SubjectId',
      'settings.sf.subjectId.placeholder': 'e.g. 5cjcchom7v',
      'settings.sf.subjectId.hint': 'Account id; run window.subjectInfo.subjectId in the console. Leave empty to clear.',
    };

    // Top-up pages (opens in a new tab). The SiliconFlow billing page is
    // used when the current provider is siliconflow; otherwise DeepSeek.
    const DEEPSEEK_TOP_UP_URL = 'https://platform.deepseek.com/top_up';
    const SILICONFLOW_TOP_UP_URL = 'https://cloud.siliconflow.cn/me/expensebill';
    const topUpUrl = (provider) =>
      provider === 'siliconflow' ? SILICONFLOW_TOP_UP_URL : DEEPSEEK_TOP_UP_URL;
     ...
     */
    function BreathingReminder({ t, provider }) {
    ...
    function BalanceCard({ wide, t, refresh }) {
      ...
      // yearly usage once on every page load (DeepSeek only — SiliconFlow has
      // no 12-month usage source). Balance is freshened right away by the 60s
      // poll below. Note: this effect deliberately does NOT depend on `snap`
      // — it runs once on mount only, avoiding a setSnap → re-run → setSnap
      // loop that would stall the card.
      useEffect(() => {
        (async () => {
          let provider = null;
          try {
            const s = await refresh({ refresh: false });
            if (s && s.ok && s.value) {
              setSnap(s.value);
      ...
      // Watch for a provider switch (the user changed models). Every few
      // seconds we ask the host for the cached numbers plus the currently
      // detected provider (zero network cost); when the provider no longer
      // matches what the card shows, force a real refresh immediately instead
      // of waiting for the next poll tick. lastHandledProvider guards against
      // re-triggering while a forced refresh is still settling.
      // A ref (not a state dep) holds the last shown provider so this effect
      // keeps a single stable interval instead of re-creating it on every
      // snap change.
      const lastHandledProvider = useRef(null);
      const lastShownProvider = useRef(null);
      if (snap && snap.provider) lastShownProvider.current = snap.provider;
      useEffect(() => {
        const watch = async () => {
          try {
            const s = await refresh({ refresh: false });
            if (!s || !s.ok || !s.value) return;
            const current = s.value.currentProvider || s.value.provider;
            if (!current) return;
            if (current === lastShownProvider.current) {
              lastHandledProvider.current = current;
              return;
            }
            if (lastHandledProvider.current === current) return;
            lastHandledProvider.current = current;
            const fresh = await refresh({ refresh: true });
            if (fresh && fresh.ok && fresh.value) setSnap(fresh.value);
          } catch {
            // keep the last known numbers
          }
        };
        const timer = window.setInterval(watch, 5000);
        return () => window.clearInterval(timer);
      }, [refresh]);
      ...
      const doRefresh = useCallback(async () => {
        ...
        try {
          const s = await refresh({ refresh: true });
          if (s && s.ok && s.value) setSnap(s.value);
          // DeepSeek only: also refresh the 12-month usage. SiliconFlow has
          // no usage source, so skip it there.
          if (!(s && s.ok && s.value && s.value.provider === 'siliconflow')) {
            const u = await refresh('usage', { refresh: true });
            if (u && u.ok && u.value) setUsage(u.value);
            else if (u && !u.ok && u.error) setUsage({ error: u.error.message });
          }
      ...
      if (!wide) {
        return jsx(
          'div',
          {
            ...
            onClick: () => window.open(topUpUrl(snap && snap.provider), '_blank', 'noreferrer'),
      ...
      // SiliconFlow has no 12-month usage source — hide the yearly rows.
      const isSiliconFlow = snap ? snap.provider === 'siliconflow' : false;
      ...
      return jsxs('div', {
        ...
        children: [
          snap && snap.total < lowThreshold
            ? jsx(BreathingReminder, { t, provider: snap.provider })
            : null,
          jsxs('div', {
            ...
            children: [
              jsx('span', {
                style: labelStyle,
                children: `${snap && snap.provider === 'siliconflow' ? '硅基流动' : 'DeepSeek'} ${t('balance.label')}`,
              }),
              jsx('strong', {
          ...
          jsxs('div', {
            ...
            children: [
              ...
              jsx('a', {
                href: topUpUrl(snap && snap.provider),
                target: '_blank',
                rel: 'noreferrer',
          ...
          // Hide the three yearly-usage rows (tokens / cost / last-update)
          // when using SiliconFlow — there is no usage data source for it.
          return !(isSiliconFlow && index >= 4 && index <= 6);
        }),
      });
    }
    ...
    function SettingsPage({ t, refresh }) {
      ...
      const [sfCookie, setSfCookie] = useState('');
      const [sfSubjectId, setSfSubjectId] = useState('');
          ...
          try {
            ...
            if (r && r.ok && r.value) {
              ...
              setMeta({
                ...
                sfCookieConfigured: v.siliconflowCookieConfigured === true,
                sfSubjectId: v.siliconflowSubjectId || null,
              });
            }
          } catch {
            // keep the defaults
      ...
      // If the user pastes a cURL command (from DevTools → Network → Copy as
      // cURL) into the cookie field, extract just the Cookie header value so
      // they never have to hand-copy the long cookie string.
      const normalizeCookieInput = (raw) => {
        const input = raw.trim();
        if (!/^curl\s/i.test(input)) return raw; // not a cURL command, keep as-is
        const m = input.match(/-H\s+["']Cookie:\s*([^"']+)["']/i);
        if (!m) return raw; // no Cookie header found, keep as-is so the user sees it
        return m[1].trim();
      };
      ...
      const persist = async () => {
        ...
        // SiliconFlow login cookie + subject id: always send (empty string
        // clears the saved value, non-empty overwrites).
        patch.siliconflowCookie = sfCookie.trim();
        patch.siliconflowSubjectId = sfSubjectId.trim();
        setSaving(true);
        setStatus(null);
        try {
      ...
      const sfCookieStatusText = meta
        ? meta.sfCookieConfigured
          ? t('settings.sf.cookie.configured')
          : t('settings.sf.cookie.unset')
        : '—';
              ...
              jsxs('div', {
                ...
                children: [
                  ...
                  jsx('input', {
                    type: 'password',
                    value: sfCookie,
                    placeholder: t('settings.sf.cookie.placeholder'),
                    onChange: (e) => setSfCookie(normalizeCookieInput(e.target.value)),
                    style: settingsInputStyle,
                    autoComplete: 'off',
                    spellCheck: false,
                  }),
                  jsx('p', { style: settingsHintStyle, children: t('settings.sf.cookie.hint') }),
                  jsx('p', { style: settingsHintStyle, children: t('settings.sf.cookie.howto') }),
                ],
              }),
              jsxs('div', {
                style: settingsFieldRowStyle,
                children: [
                  jsxs('label', {
                    style: settingsFieldLabelStyle,
                    children: [
                      t('settings.sf.subjectId'),
                      meta && meta.sfSubjectId ? ` — ${meta.sfSubjectId}` : '',
                    ],
                  }),
                  jsx('input', {
                    type: 'text',
                    value: sfSubjectId,
                    placeholder: t('settings.sf.subjectId.placeholder'),
                    onChange: (e) => setSfSubjectId(e.target.value),
                    style: settingsInputStyle,
                    autoComplete: 'off',
                    spellCheck: false,
                  }),
                  jsx('p', { style: settingsHintStyle, children: t('settings.sf.subjectId.hint') }),
                ],
              }),
            ],
          }),
